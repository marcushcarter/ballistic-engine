...
Error GeometryPool::allocate()
{
    ...
    address_buffer = dd->buffer_create({ .size = (VkDeviceSize)sizeof(GeometryAddresses), .usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, .device_local = false, .host_visible = true, .pool = dd->buffer_geometry_pool, .name = "geo_addresses" });
...
uint32_t GeometryPool::load(Guid p_guid, const std::filesystem::path& p_path)
{
    ...
    if (ph.tri_count != ph.index_count/3 || vtx_bytes + idx_bytes + tri_bytes + slot_bytes + clus_bytes != blocks_size || payload_off + blocks_size > bytes.size()) {
    ...
    if (id >= MAX_MESHES || vertex_head + ph.vertex_count > MAX_VERTS || index_head + ph.index_count > MAX_INDICES || tri_head + ph.tri_count > MAX_TRI_SLOTS || slot_table_head + ph.slot_table_count > MAX_SLOT_TABLE || cluster_head + ph.cluster_count > MAX_CLUSTERS) {
    ...
    // std::memcpy(&m.pos_min, ph.pos_min, sizeof(ph.pos_min));
    // std::memcpy(&m.pos_extent, ph.pos_extent, sizeof(ph.pos_extent));
    // std::memcpy(&m.uv_min, ph.uv_min, sizeof(ph.uv_min));
    // std::memcpy(&m.uv_extent, ph.uv_extent, sizeof(ph.uv_extent));
    // std::memcpy(&m.bounds_sphere, ph.bounds_sphere, sizeof(ph.bounds_sphere));
    ...
    // upload mesh buffer at offset "id * sizeof(LMesh)" for one mesh (sizeof(LMesh))
    by_guid.emplace(p_guid, id);

    log_write("GeometryPool: loaded %s verts=%u tris=%u clusters=%u id=%u", p_path.string().c_str(), ph.vertex_count, ph.tri_count, ph.cluster_count, id);
    return id;
}
...
{
    vertex_head = skin_head = index_head = slot_table_head = cluster_head = 0;
    meshes.clear();
    free_meshes.clear();
    by_guid.clear();
}
