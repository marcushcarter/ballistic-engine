...
void ClusterCullFeature::_create_clear_visible_pass()
{
    ...
    clear_visible_pass.setup = [](RenderGraph::Builder& b) {
...
void ClusterCullFeature::_create_cluster_expand_args_pass()
{
    ...
    cluster_expand_args_pass.setup = [](RenderGraph::Builder& b) {
        ...
        b.write_buffer("ClusterCullArgs", VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT);
    };
...
void ClusterCullFeature::_create_cluster_expand_pass()
{
    ...
    cluster_expand_pass.setup = [this](RenderGraph::Builder& b) {
        ...
        drivers::DeviceDriverVulkan::BufferCreateInfo cluster_refs_ci{};
        cluster_refs_ci.size = (VkDeviceSize)ctx->frame->cluster_ref_capacity * sizeof(uint64_t);
        cluster_refs_ci.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
    ...
    cluster_expand_pass.execute = [this](RenderGraph::CommandList& cl) {
        ...
        pc.args_addr = cull_args->device_address;
